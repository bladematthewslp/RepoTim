...
void NinjaLogic::hit(GameObject* character, Attacks::Name attackName)
{
	...
	else if(mIsGrounded == false)
	{
		...
		if(mGameObject->mState->getName() == "CStateNinjaHitAir")
		{
			...
			if(attackType.mCanSendToGround == true)
			{
				mGameObject->mState->exit(mGameObject);
				mGameObject->mState = std::unique_ptr<CState>(new CStateNinjaHitAirToGround(mGameObject)).release();
				dynamic_cast<CStateNinjaHitAirToGround*>(mGameObject->mState)->entry(character);
