...
void SlashBoxBoxCollider::onCollisionEnter(Grid& grid, BoxColliderComponent* other)
{
	...
	if(other->mGameObject->mLayerName == Layer::Enemy)
	{
		...
		if(logic->getAttack() != Attacks::PLAYER_CHOPPERSTYLE && logic->getAttack() != Attacks::PLAYER_HAILBRINGER)
		System::removeGameObject(mGameObject);
	}
	
}

void SlashBoxBoxCollider::onCollisionStay(Grid& grid, BoxColliderComponent* other)
{
	SlashBoxLogic* logic = dynamic_cast<SlashBoxLogic*>(mGameObject->mLogicComponent);
	NinjaLogic* ninjaLogic = dynamic_cast<NinjaLogic*>(other->mGameObject->mLogicComponent);
