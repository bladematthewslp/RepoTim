...
WorldForest::WorldForest(Scene::Context& context )
	: World(context)
{
	// init stuff here
...
void WorldForest::init()
{
	System::mMusicPlayer.play(Music::WoodsTheme);

	mGrid = std::unique_ptr<Grid>(new Grid(Maps::Cavern)).release();
	...
	GameObjectDesc backgroundDesc(	"Background", 
									sf::RectangleShape(sf::Vector2f(512, 720)), 
									Layer::Background, 
	...
	for(int i = 0; i < 6; i++)
	{
		...
		mBackground[i]->mRenderComponent->mSprite.setTexture(&RenderComponent::mTextureHolder.get(Textures::Background_Rounded));
		mBackground[i]->setPosition( i * 512, 0);
...
/*
bool WorldForest::handleEvent(sf::RenderWindow& window, sf::Event& event)
{
...
*/
/*
bool WorldForest::handleInput(sf::Event& event)
{
...
unsigned int WorldForest::update(sf::Time dt)
{
