	...
	, timer(0)
{
	...
	mSpawnPos[Attacks::PLAYER_IMPACT][0] =  sf::Vector2f(player->getPosition() + sf::Vector2f(-93, -55));
	mSpawnPos[Attacks::PLAYER_IMPACT][1] =  sf::Vector2f(player->getPosition() + sf::Vector2f(-80, -55));
	mSpawnPos[Attacks::PLAYER_CHOPPERSTYLE][0] =  sf::Vector2f(player->getPosition() + sf::Vector2f(-120, -100));
	mSpawnPos[Attacks::PLAYER_CHOPPERSTYLE][1] =  sf::Vector2f(player->getPosition() + sf::Vector2f(-96, -100));
...
void SlashBoxLogic::init(int dir, Attacks::Name attack, int timeToLast)
{
	...
	else if(attack == Attacks::PLAYER_HAILBRINGER)
	{
		if(dir == 1)
		{
			position = mSpawnPos[Attacks::PLAYER_HAILBRINGER][0];
		}
		else
		{
