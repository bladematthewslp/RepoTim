...
function enemyShip(properties)
{
	...
	this.patternIndex = 0;
	var patternStep = 0;
	this.pushPattern = function(properties)
	{
		var size = this.patterns.length;
		this.patterns[size] = properties;
	}
	
	this.movementActive = false;
	
	this.updateMovement = function(otherShip, patternIndex)
	{
		if(this.movementActive == true)
		{
		var dirVector = new THREE.Vector3();
		dirVector.subVectors(this.patterns[this.patternIndex].dest, this.mesh.position);
		dirVector.normalize();
		this.mesh.translateOnAxis(dirVector, this.patterns[this.patternIndex].speed);
		if(this.mesh.position.distanceTo(this.patterns[this.patternIndex].dest) < 1)
		{
					this.patternIndex = this.patternIndex + 1;
					if(this.patternIndex % this.patterns.length == 0)
					{
						this.patternIndex = 0;
...
function beam()
{
	...
	this.beamMaterial = new THREE.MeshBasicMaterial( { color: 0x1d7ada, transparent: true, opacity: 0.0, alphaTest: 0.0 } );

	this.mesh = new THREE.Mesh( this.beamGeometry, this.beamMaterial );		
	...
	{
		return (this.timer > 120 && this.timer < 200 || this.timer >= 250);
	}
